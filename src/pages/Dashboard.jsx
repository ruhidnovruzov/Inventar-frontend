...
function Modal({ isOpen, onClose, data, loadingModalData }) {
    ...
    if (loadingModalData) {
    ...
    } else if (data.name.includes('Ümumi Kompüter Sayı') || data.name.includes('Ümumi Monoblok Sayı')) { // Bu sətiri dəyişdirdim
        // Ümumi kompüter sayı və Monoblok sayı üçün qruplaşdırma məntiqi
        ...
        const sortProcessors = (details) => {
            ...
            return [...details].sort((a, b) => {
                ...
                // Hər iki element prioritet siyahıdadırsa
                if (indexA !== -1 && indexB !== -1) {
                    return indexA - indexB;
                }
                // Yalnız A elementi prioritet siyahıdadırsa, A-nı yuxarıya qoy
                if (indexA !== -1) return -1;
                // Yalnız B elementi prioritet siyahıdadırsa, B-ni yuxarıya qoy
                if (indexB !== -1) return 1;
                // Heç biri prioritet siyahıda deyilsə, alfabetik olaraq sırala
            ...
            <div className="space-y-4">
                {sortedDetails.map((param, index) => {
                    ...
                    return (
                        <div
                            key={index}
                            className={`p-4 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors border-2 ${borderColor}`}
                        >
                            <h4 className={`font-semibold text-lg mb-2 text-gray-800`}>{param.parametrAd}</h4>
...
function Dashboard() {
    const [stats, setStats] = useState({
        ...
        laboratoriya_computer_count: 0, // Laboratoriya kompüterləri üçün əlavə kateqoriya
    ...
    const statsData = [
        ...
        { name: 'Auditoriya otaqları üzrə kompüter sayı', count: stats.auditoriya_computer_count, icon: Laptop, color: 'bg-teal-500', apiPath: null, details: [] },
        //Laboratoriya kompüterləri üçün əlavə kateqoriya
